...
async function createAsset(db, data) {
  ...
  await db.prepare("INSERT INTO assets (id, name, type, bank, balance, include_in_total, billing_day, payment_day, credit_limit, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, date('now'))").bind(id, data.name, data.type, data.bank, data.balance, data.include_in_total ?? 1, data.billing_day ?? null, data.payment_day ?? null, data.credit_limit ?? 0).run();
  return id;
}
