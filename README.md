...
- 自訂分類（名稱、排序、icon）
- 定期項目（每月 / 每週 / 每年自動產生）；有設截止日期的一次列出剩餘所有期數，沒有截止日期的只顯示最近 6 期
- 點擊列表整列直接開啟編輯 modal，不額外顯示按鈕；刪除在 modal 內
- 月曆上點擊已選取的日期會直接開啟新增紀錄
...
### 快速新增（手機）
- `/add` 路由（SPA 內的一頁，可加到手機主畫面），全螢幕無瀏覽器 UI
- 內建 PIN 登入，自動從 API 載入個人分類與帳戶
- 步驟式介面：金額 → 類型/分類 → 名稱/日期 → 帳戶
- 支援支出、收入、轉帳

### 登入與密碼
- PIN 登入，登入狀態依裝置分流保存：手機（含加到主畫面的 PWA）跨開關機保留；電腦版關閉分頁/瀏覽器就需要重新登入（避免公用電腦風險）
- 「設定」內可自助更新登入密碼，不需要進 Cloudflare 後台操作
...
| POST | `/api/auth/change-pin` | 自助改密碼（需 `CF_API_TOKEN`/`WORKER_NAME` secret） |
